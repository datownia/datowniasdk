	...
	public void run() 
	{
		try
		{
			...
			if(repository.doesDatabaseExist())
			{
				...
				Logger.d("datownia", "attempting to sync database tables");
			
				if(getConnectivityHelper().isNetworkAvailable())
					appService.synchroniseDb();
					Logger.d("datownia", "synced database tables");
			}
			else
			{
				//phone has no database from datownia yet, so download it from datownia
				Logger.d("datownia", "attempting to download database tables");
				appService.downloadDb();
				Logger.d("datownia", "downloaded the database");
			} 
		}
		catch(Exception e)
		{
